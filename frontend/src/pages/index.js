  ...
  return (
    <AuthWrapper>
      ...
      <main className="h-screen pt-16 relative">
        ...
        {/* Album Filter - Floating Top Right - Modern Glass Style */}
        <div className="absolute top-20 right-4 z-40 bg-black/40 backdrop-blur-xl border border-white/10 rounded-full flex items-center gap-3 px-4 py-2 shadow-lg shadow-black/20 hover:bg-black/50 transition-colors group">
          ...
          <div className="relative">
            <select
              className="bg-transparent text-white text-sm font-medium outline-none cursor-pointer appearance-none pr-8 py-1"
              value={selectedAlbum}
              onChange={handleAlbumChange}
            >
              <option value="All" className="bg-zinc-900 text-white py-2">All Photos</option>
              {albums.map(album => (
                <option key={album} value={album} className="bg-zinc-900 text-white py-2">{album}</option>
              ))}
            </select>
            {/* Custom Chevron */}
